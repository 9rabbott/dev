...
def test_stop_unregistered_mock(mocker: MockerFixture) -> None:
    """Calling mocker.stop() with a mock not created by mocker should raise
    ValueError with a helpful message."""
    unrelated_mock = MagicMock()

    with pytest.raises(ValueError, match="This mock object is not registered"):
        mocker.stop(unrelated_mock)


def test_stop_already_stopped_mock_message(mocker: MockerFixture) -> None:
    """Calling mocker.stop() twice on the same mock should raise ValueError
    ...
    m = mocker.patch.object(Foo, "bar", return_value=0)
    mocker.stop(m)
